...
(function (root) {
  ...
  const EN = {
    ...
    '每家可以登记多个账号，并聚合成一个账号池；列表显示额度、顺位和用完提醒。按住左边的把手拖动可以调整使用顺序，达到上限才提示切到下一个，TokenPulse 不会改写 CLI 登录。点铅笔可以给账号起名字，点「添加账号」可继续添加。': 'Each provider can have several accounts grouped into one pool. The list shows quota, order, and depletion warnings. Drag the handle to set the order; TokenPulse only suggests the next account at the limit and never changes the CLI sign-in.',
    '额度已用完 · 应切换': 'Quota exhausted · switch now', '暂无额度数据': 'No quota data', '用完再换': 'Use until exhausted', '全部用完': 'All exhausted', '预计会用完': 'Likely to run out',
    '全部账号等待重置': 'All accounts are waiting for reset', '按设置顺序依次使用，不做轮询切换': 'Use accounts in the configured order; no round-robin switching',
    '尚未取得这个产品的账号额度。登录官方 CLI 或添加账号后会显示在这里。': 'No quota has been collected for this product yet. Sign in to the official CLI or add an account.',
    '查看号池': 'View pool', '当前账号耗尽后，提示切到下一个；不会修改 CLI 登录': 'Suggest the next account only after exhaustion; never change the CLI sign-in',
    '按设置顺序依次使用；当前账号耗尽才提示切到下一个，不会改写 CLI 真实账号配置。': 'Use accounts in the configured order. The next account is suggested only after exhaustion; the real CLI account configuration is never changed.',
    ...
    '未命名账号': 'Unnamed account', '本软件已保存': 'Saved in TokenPulse only', '重新授权': 'Reauthorize', '完全删除 TokenPulse 记录': 'Permanently delete TokenPulse record', '确认完全删除': 'Confirm permanent deletion', '已隐藏': 'Hidden', '上移': 'Move up', '下移': 'Move down', '删除': 'Delete', '恢复': 'Restore',
    ...
    '删除对话': 'Delete conversation', '永久删除这段会话，无法撤销': 'Permanently delete this session. This cannot be undone.', '永久删除这段对话？': 'Permanently delete this conversation?',
    '已删除对话。': 'Conversation deleted.', '已完全删除 TokenPulse 账号': 'TokenPulse account permanently deleted.', '重新授权登录的账号与目标账号不一致，原账号没有改变。': 'The reauthorized account does not match the target account. The original account was not changed.', '将删除 TokenPulse 账号、保存的凭据和该账号额度历史；不会修改 CLI 文件。再点一次确认。': 'This deletes the TokenPulse account, saved credentials, and its quota history. CLI files are not changed. Click again to confirm.', '删除对话失败，请重试。': 'Could not delete the conversation. Please try again.', '这段会话正在回复，请先停止再删除': 'This conversation is still replying. Stop it before deleting.',
    '找不到 Claude Code 的会话记录文件': 'Could not find the Claude Code conversation file.', '没找到 Codex CLI，无法删除会话': 'Codex CLI was not found, so the conversation cannot be deleted.', '没找到 Grok Build，无法删除会话': 'Grok Build was not found, so the conversation cannot be deleted.',
    'Claude Code 没有普通对话删除命令；TokenPulse 会删除本机的会话记录文件和同名附属目录。此操作无法撤销。': 'Claude Code has no command for deleting a regular conversation. TokenPulse will remove its local transcript and matching sidecar directory. This cannot be undone.',
    '将调用 codex delete <会话 ID> 永久删除这段对话。此操作无法撤销。': 'TokenPulse will run codex delete <session ID> to permanently delete this conversation. This cannot be undone.',
    '将调用 grok sessions delete <会话 ID> 永久删除这段对话。此操作无法撤销。': 'TokenPulse will run grok sessions delete <session ID> to permanently delete this conversation. This cannot be undone.',
    ...
    '按住账号左右拖动，可以调整顺序': 'Drag an account sideways to reorder',
    '拖动下方滚动条查看更多账号': 'Drag the scrollbar below to see more accounts',
    '作者': 'Author',
  ...
  const PATTERNS = [
    P(N + ' 个账号 · 周池剩余 ' + N + '%', m => `${m[1]} accounts · ${m[2]}% weekly pool remaining`),
    P(N + ' 个账号 · 用完再换', m => `${m[1]} accounts · use until exhausted`),
    P(N + ' 个账号', m => `${m[1]} accounts`),
    P('当前顺位 · 约 (.+)后用完', m => `Current · runs out in about ${v(m[1])}`),
    P('当前顺位 · 剩余 (.+)%', m => `Current · ${m[1]}% remaining`),
    P('下一个 · 剩余 (.+)%', m => `Next · ${m[1]}% remaining`),
    P('排队 · 剩余 (.+)%', m => `Queued · ${m[1]}% remaining`),
    P('5 小时剩 (.+)%', m => `${m[1]}% left in 5-hour window`),
    P('周剩 (.+)%', m => `${m[1]}% weekly remaining`),
    P('(.+)额度合计剩余', m => `${v(m[1])} total remaining`),
    P('(.+) 在当前顺位', m => `${m[1]} is current`),
    P('按近期节奏约 (.+)后达到上限', m => `At the recent pace, the limit is reached in about ${v(m[1])}`),
    P('(.+) 账号池', m => `${m[1]} account pool`),
    P('(\\d+) 分钟', m => `${m[1]} min`),
